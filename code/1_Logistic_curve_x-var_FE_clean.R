...
p1 <- ggplotly(
  ggplot(data = data, aes(x = factor(year), y = density_psqkm, 
    ...
    labs(
      ...
      x = "Year",
      y = "Population Density (people / sqkm)"
    ) +
    theme_minimal() +
    create_theme1(14) +
    scale_color_manual(values = country_colors) +
    # Display every 5th year of x-axis
...
htmlwidgets::saveWidget(p1, here::here("plots/population_density_over_time.html"), selfcontained = TRUE)

p1 <- ggplotly(
  ggplot(data = data, aes(x = factor(year), y = GDP_PPP_pcap, 
                          colour = country_name, group = country_name)) +
    geom_line() +
    geom_point(aes(shape = country_name), size = 1.5, alpha = 0.7) +
    geom_text(
...
# Save to html self-contained
htmlwidgets::saveWidget(p1, here::here("plots/gdp_over_time.html"), selfcontained = TRUE)

p1 <- ggplotly(
  ggplot(data = data, aes(x = factor(year), y = Gini_01, 
                          colour = country_name, group = country_name)) +
    geom_line() +
    geom_point(aes(shape = country_name), size = 1.5, alpha = 0.7) +
    geom_text(
...
# Simple Logistic with Gini: Amplify/ dampens the effect of GDP_PPP_pcap on ES_pcap
...
nls2_fit <- nls2(
  ES_pcap ~ logistic_model_1(GDP_PPP_pcap, density_psqkm, alpha_0, alpha_1, xmid, scal), data = data1,
  start = data.frame(
...
(bicmodel1_1 <- BIC(nls2_fit))
(bicmodel1_2 <- BIC(nlsLM_fit))

# Fit stats of best fitting model
fit_stats1 <- data.frame(
  Model = "Logistic 1: Without Gini",
  AIC = AIC(nlsLM_fit),
  BIC = BIC(nlsLM_fit)#,
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Logistic 1 Model without Gini",
...
####################################################################################
# Fit the Logistic 2 model using nls2 with a wide range of starting values
####################################################################################
...
fit_stats2 <- data.frame(
  Model = "Logistic 2: Simple with Gini",
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Logistic 2 Model: Simple with Gini",
    x = "GDP per capita (USD)",
    y = "Energy Service per Capita (passenger km / capita)",
    color = "Country",
    linetype = "Data Type"
...
nls2_fit_3 <- nls2(
  ...
  data = data1,
  start = data.frame(
    alpha_0 = seq(1000, 30000, length.out = 10),     # base level of ES_pcap
    alpha_1 = seq(-5, 5, length.out = 10),           # moderate influence of density
    xmid     = seq(5000, 40000, length.out = 10),   # GDP midpoint where curve inflects
    scal     = seq(2000, 20000, length.out = 10),    # scale (steepness) > 0
    beta_1   = seq(-4, 4, length.out = 10),         # Gini effect on exponent
    beta_0   = seq(0.5, 5, length.out = 10)          # base exponent
  )
  ,
  algorithm = "brute-force"
)
...
nlsLM_fit_3 <- nlsLM(
  ...
  data = data1,
  start = best_start_vals,
  lower = c(alpha_0 = 0, alpha_1 = -Inf, xmid = 22000, scal = 0, beta_1 = -Inf, beta_0 = -Inf),
  control = nls.lm.control(maxiter = 500, ftol = 1e-8)
)
...
fit_stats3 <- data.frame(
  Model = "Logistic 3: Advanced with Gini",
...
nls2_fit_4 <- nls2(
  ES_pcap ~ logistic_model_4(GDP_PPP_pcap, density_psqkm, Gini_01, alpha_0, alpha_1, xmid, scal, beta_1),
  data = data1,
  start = expand.grid(
    alpha_0 = seq(100, 75000, length.out = 5),
    alpha_1 = seq(-10, 10, length.out = 5),
    xmid = seq(5000, 70000, length.out = 5),
    scal = seq(0, 100000, length.out = 5),
    beta_1 = seq(-10000, 10000, length.out = 5)
  ),
  algorithm = "brute-force"
)
...
fit_stats4 <- data.frame(
  Model = "Logistic 4: Advanced with Gini Interaction",
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Logistic 4 Model: Advanced with Gini Interaction",
    x = "GDP per capita (USD)",
    y = "Energy Service per Capita (passenger km / capita)",
    color = "Country",
    linetype = "Data Type"
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_logistic_4_fe.html"),
    selfcontained = TRUE
  )

# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_logistic_4_fe.png"),
  plot = p1,
  width = 16, height = 16, dpi = 150
)

#################################################################################
# FIT LOGISTIC MODEL 5
#################################################################################
...
param_grid <- expand.grid(
  alpha_0 = seq(1000, 50000, length.out = 5),       # Base level of energy service
  alpha_1 = seq(-5, 5, length.out = 5),              # Effect of population density
  xmid    = seq(10000, 60000, length.out = 5),      # Inflection point of logistic curve
  beta_0  = seq(0.5, 2, length.out = 3),            # Base exponent
  beta_1  = seq(-1, 1, length.out = 3),             # Gini effect on exponent
  gamma_0 = seq(1000, 50000, length.out = 5),       # Base scale of logistic curve
  gamma_1 = seq(-500, 500, length.out = 5)          # Gini effect on scale
)
...
nls2_fit_5 <- nls2(
  ES_pcap ~ logistic_model_5(GDP_PPP_pcap, density_psqkm, Gini_01,
                             alpha_0, alpha_1, xmid, beta_0, beta_1, gamma_0, gamma_1),
  data = data1,
  start = param_grid,
  algorithm = "brute-force"
)
...
fit_stats5 <- data.frame(
  Model = "Logistic 5: Advanced with Gini Interaction",
...
# Tidy the model output (example shown with `fit1`; replace with your model if needed)
tidy_fit5 <- broom.mixed::tidy(fit1)
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Logistic 5 Model: Advanced with Gini Shape and Scale",
    x = "GDP per capita (USD)",
    y = "Energy Service per Capita (passenger km / capita)",
    color = "Country",
    linetype = "Data Type"
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_logistic_5_fe.html"),
    selfcontained = TRUE
  )
...
#################################################################################
fit_stats_logistic <- rbind(fit_stats1, fit_stats2, fit_stats3, fit_stats4, fit_stats5)
...
htmlreg(
  ...
  file = here::here("results/tidy_model_summary_logistic_all.html"),
  ...
  override.gof = list(
    ...
    c(summary(model_chosen_2)$sigma, AIC(model_chosen_2), BIC(model_chosen_2)),
    c(summary(model_chosen_3)$sigma, AIC(model_chosen_3), BIC(model_chosen_3)),
    c(summary(model_chosen_4)$sigma, AIC(model_chosen_4), BIC(model_chosen_4)),
    c(summary(model_chosen_5)$sigma, AIC(model_chosen_5), BIC(model_chosen_5))
  ),
