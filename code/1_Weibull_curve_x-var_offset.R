...
logistic_model_1 <- deriv(
  ~ (alpha_0 + alpha_1 * density_psqkm) * (1 - exp(-(GDP_PPP_pcap/ lambda)^(k * Gini))),
  namevec = c("alpha_0", "alpha_1", "lambda", "k"),
  function.arg = c("GDP_PPP_pcap", "density_psqkm", "Gini", "alpha_0", "alpha_1", "lambda", "k")
)
...
nls2_fit <- nls2(
  ES_pcap ~ logistic_model_1(GDP_PPP_pcap, density_psqkm, Gini_01, alpha_0, alpha_1, lambda, k),#(alpha_0 + alpha_1 * density_psqkm) * (1 / (1 + exp((GDP_PPP_pcap - xmid)/scal))),
  data = data1,
  start = data.frame(
    alpha_0 = seq(0, 35000, length.out = 10),
    alpha_1 = seq(-5, 5, length.out = 10),
    lambda = seq(1000, 50000, length.out = 10),
    k = seq(0.1, 5, length.out = 10)
  )  ,
...
kable(fit_stats1, digits = 3, caption = "Model Fit Statistics for Weibull 1 Model") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  save_kable(here::here("results/model_fit_statistics_weibull_1.html"))
...
# Save to HTML file
kable(tidy_fit, digits = 3, caption = "Summary of Weibull 1 Model") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  save_kable(here::here("results/tidy_model_summary_weibull1.html"))
...
historical_data %>% group_by(country_name) %>% slice_head(n = 3)
...
p1 <- ggplot() +
  # Historical lines
  # Ensure lines are connected by year order per country_name
  geom_path(
  ...
  # ) +
  # Predicted lines
  # Use Geom_path for connecting lines in the predicted data by year
  geom_path(
    data = predicted_data,
    aes(
      x = GDP_PPP_pcap,
...
#   ) +
  ...
  labs(
    title = "Predicted Energy Service per Capita using Weibull 1 Model",
