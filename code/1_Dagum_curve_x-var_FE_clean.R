...
# Define the Dagum CDF model
Dagum_model <- function(GDP_PPP_pcap, density_psqkm, a, b, c, p) {
  scale <- a + b * density_psqkm
  cdf <- (1 + (GDP_PPP_pcap / scale)^(-c))^(-p)
  return(cdf)
}
...
# Fit the model
model_dagum <- nlsLM(
  ES_pcap ~ Dagum_model(GDP_PPP_pcap, density_psqkm, a, b, c, p),
  data = data1,
  start = list(a = 10000, b = 1, c = 2, p = 2),
...
# Raw residuals
res <- residuals(model_dagum)

# Estimate standard deviation of residuals
sigma <- summary(model_dagum)$sigma
...
Dagum_model_1 <- function(GDP_PPP_pcap, density_psqkm, a, b, c, p) {
  scale <- a + b * density_psqkm
  cdf <- (1 + (GDP_PPP_pcap / scale)^(-c))^(-p)
  return(cdf)
}
...
Dagum_model_2 <- function(GDP_PPP_pcap, density_psqkm, Gini_01,a, b, c, p) {
  scale <- a + b * density_psqkm
  cdf <- (1 + (GDP_PPP_pcap / scale)^(-c * Gini_01))^(-p)
  return(cdf)
}
# 
...
nls2_fit <- nlsLM(
  ES_pcap ~ Dagum_model_1(GDP_PPP_pcap, density_psqkm, a, b, c, p),
  data = data1,
  start = list(a = 10000, b = 1, c = 2, p = 2),
  na.action = na.exclude,
  control = nls.lm.control(maxiter = 500, ftol = 1e-10),
...
fit_stats1 <- data.frame(
  Model = "Dagum model without Gini",
  AIC = AIC(nlsLM_fit),
  BIC = BIC(nlsLM_fit)#,
...
maturity_term <- with(data1, coefs["b"] + coefs["c"] * density_psqkm)
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_Dagum_1.html"),
    selfcontained = TRUE
  )
...
nls2_fit_2 <- nlsLM(
  ES_pcap ~ Dagum_model_2(GDP_PPP_pcap, density_psqkm,Gini_01, a, b, c, p),
  data = data1,
  start = list(a = 10000, b = 1, c = 2, p = 2),
  na.action = na.exclude,
  control = nls.lm.control(maxiter = 500, ftol = 1e-10),
...
fit_stats_dagum2 <- rbind(fit_stats1, fit_stats2)
...
# Compute the maturity term for each observation
maturity_term <- with(data1, coefs["a"] + coefs["c"] * density_psqkm)
...
Dagum_2_predicted <- predicted_data
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Dagum 2 Model: With Gini Simple Variant 1",
    x = "GDP per capita (USD)",
    y = "Energy Service per Capita (passenger km / capita)",
    color = "Country",
    linetype = "Data Type"
...
#################################################################################
fit_stats_dagum <- rbind(fit_stats1, fit_stats2)# fit_stats3, fit_stats4)

kable(fit_stats_dagum, digits = 3, caption = "Model Fit Statistics for Dagum Models") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  save_kable(here::here("results/model_fit_statistics_Dagum_all.html"))

# Export tidy_fit for all models to html
htmlreg(
  list(model_chosen_1, model_chosen_2),
