...
Dagum_model <-  deriv(
  ~ (a0 + a1 * density_psqkm) * (1 + (GDP_PPP_pcap / b)^(-(c * Gini)))^(-p),
  namevec = c("a0", "a1", "b", "c", "p"),
  function.arg = c("GDP_PPP_pcap", "density_psqkm", "Gini", "a0", "a1", "b", "c", "p")
)


# Fit the model
model_dagum <- nlsLM(
  ES_pcap ~ Dagum_model(GDP_PPP_pcap, density_psqkm, Gini_01, a0,a1, b, c, p),
  data = data1,
  start = list(a0 = 10000, a1 = 0.1 , b = 1, c = 2, p = 2),
...
#################################################################################
fit_stats_dagum <- rbind(fit_stats1)# fit_stats3, fit_stats4)
