...
# 
# Gompertz_model <- deriv(
#   ~ (alpha + beta1 * density_psqkm) * exp(-b * exp(-(gamma * Gini) * Year0)),
#   namevec = c("b","alpha","beta1","gamma"),
#   function.arg = c("GDP_PPP_pcap","density_psqkm","alpha","b","beta1","gamma","Gini","Year0")
#   )

Gompertz_model <- deriv(
  ~ (alpha + beta1 * density_psqkm) * exp(-b * exp(-(gamma * Gini) * Year0)),
  namevec = c("alpha", "b", "beta1", "gamma"),
...
#data_clean <- subset(data1, complete.cases(GDP_PPP_pcap, density_psqkm, Year0, Gini, ES_pcap, country_name))


fit5 <- nlme(
    ES_pcap ~ Gompertz_model(GDP_PPP_pcap, density_psqkm, Year0, alpha, b, beta1, gamma, Gini),
    data = data1,
    fixed = alpha + beta1 + b + gamma ~ 1,
    random = alpha + beta1    ~ 1 | country_name,
    start = c(alpha = 5000, beta1 = 0.1, b = 1, gamma = 0.001),
    na.action = na.omit,
