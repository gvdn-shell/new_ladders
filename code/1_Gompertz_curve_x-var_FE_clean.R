...
#   sheet = "Settings",
#   range = "L10:M270", # Change range according to the number of colours you need
#   col_names = TRUE
# ) %>%
#   dplyr::select(-Colour)
# 
...
p <- ggplot(data = data1, aes(x = GDP_PPP_pcap, y = ES_pcap, colour = Hex)) +
  geom_line() +
  geom_point(size = 1.5, alpha = 0.7) + # Removed shape mapping
  geom_text(
...
# Fit the model
model_gompertz <- nlsLM(
  ES_pcap ~ (a0 + a1 * density_psqkm) * (1 - exp(-alpha * (exp(beta * GDP_PPP_pcap) - 1))),
  data = data1,
  start = list(a0 = 1000, a1 = 0.01, alpha = 1, beta = 0.001)
)



# Raw residuals
res <- residuals(model_gompertz)

# Estimate standard deviation of residuals
sigma <- summary(model_gompertz)$sigma
...
p <- ggplot(data1, aes(x = GDP_PPP_pcap, y = std_residuals)) +
  geom_point(aes(colour = country_name), size = 1.5, alpha = 0.7) +
  geom_hline(yintercept = c(-3, 3), linetype = "dashed", color = "red") + # ±3 SD rule
  labs(
  ...
  theme_minimal() +
  create_theme1(14) + # Custom theme function
...
p <- ggplot(data1, aes(x = GDP_PPP_pcap, y = Cooks_D)) +
  geom_point(aes(colour = country_name), size = 1.5, alpha = 0.7) +
...
p <- ggplot(data = data1, aes(x = GDP_PPP_pcap, y = ES_pcap, colour = Hex)) +
  ...
  theme_minimal() +
  create_theme1(1) + # Custom theme function
  scale_color_identity() +
  theme(legend.position = "none")
...
# Gompertz without Gini


Gompertz_model_1 <- deriv(
  ~ (a0 + a1 * density_psqkm) * (1 - exp(-alpha * (exp(beta * GDP_PPP_pcap) - 1))),
  namevec = c("a0", "a1", "alpha", "beta"),
  function.arg = c("GDP_PPP_pcap", "density_psqkm", "a0", "a1", "alpha", "beta")
)


Gompertz_model_2 <- deriv(
  ~ (a0 + a1 * density_psqkm) * (1 - exp(-alpha * Gini_01 * (exp(beta * GDP_PPP_pcap) - 1))),
  namevec = c("a0", "a1", "alpha", "beta"),
  function.arg = c("GDP_PPP_pcap", "density_psqkm", "Gini_01", "a0", "a1", "alpha", "beta")
)


Gompertz_model_3 <- deriv(
  ~ (a0 + a1 * density_psqkm) * (1 - exp((-alpha * Gini_01 + b) * (exp(beta * GDP_PPP_pcap) - 1))),
  namevec = c("a0", "a1", "alpha", "b","beta"),
  function.arg = c("GDP_PPP_pcap", "density_psqkm", "Gini_01", "a0", "a1", "alpha","b", "beta")
)


Gompertz_model_4 <- deriv(
  ~ (a0 + a1 * density_psqkm) * (1 - exp(-(alpha * Gini_01 + b) * (exp(beta * GDP_PPP_pcap) - 1))),
  namevec = c("a0", "a1", "alpha", "b","beta"),
  function.arg = c("GDP_PPP_pcap", "density_psqkm", "Gini_01", "a0", "a1", "alpha","b", "beta")
)
...
nls2_fit <- nls2(
  ES_pcap ~ Gompertz_model_1(GDP_PPP_pcap, density_psqkm, a0, a1, alpha, beta),
  data = data1,
  start = data.frame(
    a0 = 1000,
    a1 = 0.01,
...
fit_stats1 <- data.frame(
  Model = "Gompertz model without Gini",
  AIC = AIC(nlsLM_fit),
  BIC = BIC(nlsLM_fit)#,
...
maturity_term <- with(data1, coefs["a0"] + coefs["a1"] * density_psqkm)
...
model_data <- 
  data1[complete.cases(data1[, c(
    "ES_pcap", "GDP_PPP_pcap", "density_psqkm"
  )]), ]
...
p <- ggplot(model_data, aes(x = predicted, y = residuals, color = country_name)) +
  geom_point() +
...
p1 <- ggplot(model_data, aes(sample = residuals)) +
  geom_qq() +
  geom_qq_line() +
...
htmlwidgets::saveWidget(p1, here::here("plots/Gompertz_qq_plot_residuals_by_country.html"))
...
fit1 <- model_chosen_1   # Replace with your fitted model if needed
...
data1_model <- full.dataset[complete.cases(full.dataset[, c(
  "ES_pcap", "GDP_PPP_pcap_thousands", "density_psqkm"
)]), ]
...
p <- ggplot(data1_model, aes(x = ES_pcap, y = predicted_ES_pcap, color = country_name)) +
...
# standardize residuals
 ...
 p0 <- ggplot(data1_model, aes(x = predicted_ES_pcap, y = residuals, color = country_name)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(
    title = "Residuals vs Fitted Values",
...
Gompertz_1_predicted <- predicted_data
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Gompertz 1 Model without Gini",
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_Gompertz_1.1.html"),
    selfcontained = TRUE
  )

# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_Gompertz_1.1_fe.png"),
  plot = p1,
  width = 16, height = 16, dpi = 150
)

################################################################################
# FIT Gompertz 2 MODEL
################################################################################
...
nls2_fit_2 <- nls2(
  ES_pcap ~ Gompertz_model_2(GDP_PPP_pcap, density_psqkm, Gini_01 ,a0, a1, alpha, beta),
  data = data1,
  start = data.frame(
    a0 = 1000,
    a1 = 0.01,
...
fit_stats2 <- data.frame(
  Model = "Gompertz 2: With Gini Simple Variant 2",
  AIC = AIC(model_chosen_2),
  BIC = BIC(model_chosen_2)#,
  #R_squared = summary(fit_weibull_4)$r.squared
)

fit_stats_gompertz2 <- rbind(fit_stats1, fit_stats2)
...
Gompertz_2_predicted <- predicted_data
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_Gompertz_2_fe.html"),
    selfcontained = TRUE
  )

# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_Gompertz_2_fe.png"),
  plot = p1,
  width = 16, height = 16, dpi = 150
)

################################################################################
# FIT Gompertz 3 MODEL
################################################################################
# Check for invalid combinations
# Create parameter grid
# param_grid <- expand.grid(
...
nls2_fit_3 <- nls2(
  ES_pcap ~ Gompertz_model_3(GDP_PPP_pcap, density_psqkm,Gini_01, a0, a1, alpha, b, beta),
  data = data1,
  start = data.frame(
    a0 = 1000,
    a1 = 0.01,
...
fit_stats3 <- data.frame(
  Model = "Gompertz 3: Advanced with Gini",
...
Gompertz_3_predicted <- predicted_data
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Logistic 3 Model: Advanced with Gini",
    x = "GDP per capita (USD)",
    y = "Energy Service per Capita (passenger km / capita)",
    color = "Country",
    linetype = "Data Type"
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_Gompertz_3_fe.html"),
    selfcontained = TRUE
  )

# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_Gompertz_3_fe.png"),
  plot = p1,
  width = 16, height = 16, dpi = 150
)

################################################################################
# FIT Gompertz 4 MODEL
################################################################################
# Check for invalid combinations
# Create parameter grid
# param_grid <- expand.grid(
...
# Fit the model using nlsLM with starting values
nlsLM_fit_4 <- nlsLM(
  ES_pcap ~ Gompertz_model_4(GDP_PPP_pcap, density_psqkm, Gini_01,a0, a1, alpha, b,beta),
  data = data1,
  start = best_start_vals,
  control = nls.lm.control(maxiter = 500, ftol = 1e-8),
  na.action = na.exclude,
...
fit_stats4 <- data.frame(
  Model = "Gompertz 4: Advanced with Gini",
...
fit4 <- model_chosen_4 #fit_weibull_4  # Replace with your fitted model if needed
# Tidy the model output (example shown with `fit1`; replace with your model if needed)
tidy_fit4 <- broom.mixed::tidy(fit4)
...
p1 <- ggplot() +
  ...
  scale_color_manual(values = country_colors) +
  labs(
    title = "Predicted Energy Service per Capita using Gompertz 4 Model: Advanced with Gini",
    x = "GDP per capita (USD)",
    y = "Energy Service per Capita (passenger km / capita)",
    color = "Country",
    linetype = "Data Type"
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_Gompertz_4_fe.html"),
    selfcontained = TRUE
  )

# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_Gompertz_4_fe.png"),
  plot = p1,
  width = 16, height = 16, dpi = 150
)
...
#################################################################################
fit_stats_Gompertz <- rbind(fit_stats1, fit_stats2, fit_stats3, fit_stats4)

kable(fit_stats_Gompertz, digits = 3, caption = "Model Fit Statistics for Gompertz Models") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  save_kable(here::here("results/model_fit_statistics_Gompertz_all.html"))

# Export tidy_fit for all models to html
htmlreg(
  list(model_chosen_1, model_chosen_2, model_chosen_3, model_chosen_4),
  file = here::here("results/tidy_model_summary_Gompertz_all.html"),
  ...
  override.gof = list(
    ...
    summary(model_chosen_3)$sigma, AIC(model_chosen_3),
    summary(model_chosen_4)$sigma, AIC(model_chosen_4)
  ),
  include.deviance = FALSE,
  # override.gof.names = c("Residual Std. Error", "AIC", "BIC"),
  
  caption = "Gompertz Model Summaries",
  caption.above = TRUE
)
...
# CREATE PLOT: GDP vs. ES_pcap (Historical vs. Predicted)
################################################################################
Gompertz_1_predicted$line_type <- "Predicted: Gompertz 1 (wo Gini)"
Gompertz_2_predicted$line_type <- "Predicted: Gompertz 2 (Gini exponent)"
Gompertz_3_predicted$line_type <- "Predicted: Gompertz 5 (Gini complex)"
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_Gompertz_best_fe.html"),
    selfcontained = TRUE
  )
...
# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_Gompertz_best_fe.png"),
  plot = p1,
  width = 16, height = 16, dpi = 150
)
