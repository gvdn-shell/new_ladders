...
gompertz_model_1 <- deriv(
  ~ (alpha_0 + alpha_1 * density_psqkm) * (1 - exp(- shape * (exp( scal * GDP_PPP_pcap) -1))),
...
nls2_fit <- nls2(
  ES_pcap ~ gompertz_model_1(GDP_PPP_pcap, density_psqkm, alpha_0, alpha_1, scal, shape), data = data1,
  start = data.frame(
    alpha_0 = seq(1000, 20000, length.out = 5),
    alpha_1 = seq(-5, 5, length.out = 5),
    scal = seq(0.00001, 0.001, length.out = 5),  # much smaller scale
    shape = seq(0.1, 2, length.out = 5)
  ) , 
  trace = TRUE,
...
nlsLM_fit <- nlsLM(
  ES_pcap ~ gompertz_model_1(GDP_PPP_pcap, density_psqkm, alpha_0, alpha_1, scal, shape),
  data = data1,
...
fit_stats1 <- data.frame(
  Model = "Gompertza 1: Without Gini",
  AIC = AIC(nlsLM_fit),
  BIC = BIC(nlsLM_fit)#,
...
htmlwidgets::saveWidget(p1, here::here("plots/gompertz1_qq_plot_residuals_by_country.html"))
...
ggplotly(p1 + theme(legend.position = "right"), tooltip = "text") %>%
  htmlwidgets::saveWidget(
    here::here("plots/predicted_ES_pcap_gompertz_1_fe.html"),
    selfcontained = TRUE
  )

# Save static PNG version
ggsave(
  filename = here::here("plots/predicted_ES_pcap_gompertz_1_fe.png"),
  plot = p1,
